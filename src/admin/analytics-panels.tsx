                ...
                <tbody>
                  {data.rows.map((r) => (
                    <tr key={r.id} className="border-b border-sand/60">
                      ...
                      <td className="py-2 pr-3 text-espresso/80 tabular-nums">
                        {nf.format(r.views)}
                      </td>
                      <td className="py-2 text-espresso/80 tabular-nums">{nf.format(r.orders)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export function EntityAnalytics({ kind, id }: { kind: "announcement" | "popup"; id: string }) {
  const [timeframe, setTimeframe] = useState("week")
  const [now, setNow] = useState(() => Date.now())
      ...
      {data === undefined ? (
        ...
        <>
          ...
          >
            ...
            {kind === "popup" && (
              <>
                <PlainStat label="Emails" value={nf.format(data.totals.emails)} />
                <PlainStat
                  label="Avg. Time"
