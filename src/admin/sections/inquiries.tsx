...
import { CONTACT_TOPICS, contactTopicLabel, type ContactTopic } from "@/lib/contact-topics"
...
function ReadFilter({ value, onChange }: { value: ReadStatus; onChange: (v: ReadStatus) => void }) {
      ...
      {opts.map((o) => (
        <button
          ...
          className={`rounded-full px-3.5 py-1.5 text-sm font-semibold transition-colors ${
            value === o.v ? "bg-brick text-cream" : "text-espresso/70 hover:bg-cream-deep"
          }`}
        >
          {o.label}
        </button>
...
function TopicFilter({
  ...
  function toggle(topic: ContactTopic) {
    onChange(
      value.includes(topic) ? value.filter((existing) => existing !== topic) : [...value, topic],
    )
  }

  return (
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border-2 border-sand bg-cream px-3 py-2">
      <span className="text-xs font-semibold tracking-wide text-espresso/50 uppercase">Topic</span>
        ...
        return (
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-semibold transition-colors ${
              active ? "bg-brick text-cream" : "bg-cream-deep text-espresso/65 hover:bg-sand"
            }`}
          >
            {topic.label}
          </button>
  ...
  return (
    <div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        />
        <SummaryCard label="Hiring" total={counts?.hiring.total} unread={counts?.hiring.unread} />
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold transition-colors ${
                tab === t.key ? "bg-brick text-cream" : "text-espresso/70 hover:bg-cream-deep"
              }`}
            >
              {t.label}
              {unread > 0 && (
...
function SummaryCard({
  ...
  return (
    <div className="rounded-2xl border-2 border-sand bg-cream px-4 py-3">
      <p className="text-xs font-semibold tracking-wide text-espresso/55 uppercase">{label}</p>
      <div className="mt-1 flex items-baseline gap-3">
        <span className="font-display text-2xl leading-none text-espresso">{total ?? "-"}</span>
        {unread !== undefined && unread > 0 && (
          <span className="text-sm font-semibold text-brick">{unread} unread</span>
        )}
      </div>
    </div>
  )
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-espresso/50">{fmtDate(r._creationTime)}</span>
                  <ReadBtn
                    read={!!r.read}
              ...
              <p className="mt-2 whitespace-pre-wrap text-espresso/85">{r.message}</p>
            </div>
          ))}
        </div>
      )}
...
function applicantName(r: { name?: string; firstName?: string; lastName?: string }): string {
...
function HiringTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      if (!matchesRead(r.read, status)) return false
      if (coffee !== "all" && (r.coffeeExperience ?? "") !== coffee) return false
      if (!needle) return true
      return [applicantName(r), r.email, r.phone, r.favoriteCoffeeShop, r.favoriteRecord]
        .join(" ")
        .toLowerCase()
        .includes(needle)
    })
            ...
            return (
              ...
              >
                  ...
                  <div>
                    <p className="flex flex-wrap items-center gap-2 font-semibold text-espresso">
                      ...
                      {r.coffeeExperience && (
                        <span className="ml-1 rounded-full bg-gold/30 px-2 py-0.5 text-xs font-semibold text-espresso">
                          {r.coffeeExperience === "yes" ? "Coffee exp ✓" : "No coffee exp"}
                        </span>
                      )}
                      {availability.map((a) => (
                        <span
                    ...
                    {(r.favoriteCoffeeShop || r.favoriteRecord) && (
                      <p className="mt-1 text-sm text-espresso/70">
                        {r.favoriteCoffeeShop && <>☕ {r.favoriteCoffeeShop}</>}
                        {r.favoriteCoffeeShop && r.favoriteRecord && " · "}
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3">
                  {r.resumeUrl && (
                    <a href={r.resumeUrl} target="_blank" rel="noreferrer" className={btn.small}>
                ...
                {openId === r._id && r.details && <Details details={r.details} />}
              </div>
            )
          })}
        </div>
...
function Details({ details }: { details: unknown }) {
  const entries =
    details && typeof details === "object" ? Object.entries(details as Record<string, unknown>) : []
  if (entries.length === 0) {
    return <p className="mt-3 text-sm text-espresso/55">No extra details captured.</p>
  }
  return (
...
function CapturesTab() {
  ...
  function exportCsv() {
    ...
    const body = filtered.map((r) => [r.email, r.source, new Date(r._creationTime).toISOString()])
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r._id} className="border-t border-sand bg-cream">
                  ...
                  <td className="px-4 py-2 text-espresso/55">{fmtDate(r._creationTime)}</td>
                  <td className="px-4 py-2 text-right">
                    <DeleteBtn
                      onClick={() =>
                        confirmThen(
                          "Delete this email capture?",
...
function SearchBar({
  ...
  return (
    <div className={inline ? "" : "flex items-center gap-3"}>
      ...
      />
      {!inline && <span className="text-sm text-espresso/55">{count} result(s)</span>}
    </div>
  )
}
