...
import { DietaryTagPicker } from "@/admin/dietary-tag-picker"
...
export function MenuManager({ menu }: { menu: Menu }) {
  ...
  const reorderItems = useMutation(api.menu.reorderItems).withOptimisticUpdate((store, args) => {
    ...
    if (!current) return
    const items = new Map(
      current.sections.flatMap((section) => section.items.map((item) => [item._id, item] as const)),
    )
    store.setQuery(
      api.menu.getMenu,
      { menu },
  ...
  async function dropItem(targetSectionId: Id<"menuSections">, targetIndex: number) {
    ...
    try {
      ...
      setReorderError(error instanceof Error ? error.message : "Couldn't save the new item order.")
    } finally {
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          {page && (
            <p className="mt-1 font-groovy text-xs tracking-[0.25em] text-terracotta uppercase">
              {page.eyebrow}
            </p>
          )}
        </div>
      ...
      {analyticsMenu && (
        <div className="mt-5 rounded-2xl border-2 border-sand bg-cream">
          ...
          >
            <span className="font-groovy text-sm tracking-[0.2em] text-terracotta uppercase">
              Menu Analytics
            </span>
        ...
        {sections.length === 0 && (
          <div className="rounded-2xl border-2 border-dashed border-sand p-6">
            <p className="text-espresso/60">
              No sections yet. Add one to start building this {isMerch ? "merch catalog" : "menu"},
              or seed the original content in one click.
            </p>
            <button
              type="button"
...
function Loading({ menu }: { menu: Menu }) {
  return (
    <div>
      <h1 className="font-display text-4xl text-espresso capitalize">{menu} Menu</h1>
...
function MenuPdf({ menu, pdf }: { menu: Menu; pdf: { url: string | null; name: string } | null }) {
    ...
    <div className="mt-5 flex flex-wrap items-center gap-3 rounded-2xl border-2 border-sand bg-cream p-4">
      ...
      {pdf && (
        <button
          type="button"
          className={btn.danger}
          onClick={() => confirmThen("Remove the menu PDF?", () => void removeMenuPdf({ menu }))}
        >
          Remove
        </button>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder={
              warnToast ? "https://order.toasttab.com/online/…" : "https://example.com/product"
            }
            inputMode="url"
          />
          {warnToast && orderUrl.trim() && !isToastUrl(orderUrl) ? (
            <p className="mt-1 text-xs font-semibold text-brick">
              ⚠ This doesn't look like a Toast link (toasttab.com). Double-check it before saving -
              the item's Order button points here.
            </p>
          ) : (
            <p className="mt-1 text-xs text-espresso/50">{linkHelp}</p>
          )}
        </div>
        <div>
