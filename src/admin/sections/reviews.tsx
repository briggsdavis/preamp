  ...
  const visible = useMemo(() => {
    ...
    list =
      view === "archived"
        ...
        : list.filter((r) => r.status !== "archived")
    if (view === "active" && newOnly) list = list.filter((r) => r.status === "pending")
    if (itemFilter !== "all") list = list.filter((r) => r.menuItemName === itemFilter)
    const sorted = [...list]
    sorted.sort((a, b) => {
      switch (sort) {
        case "oldest":
  ...
  return (
    <div>
        ...
        {(["active", "archived"] as const).map((v) => (
          <button
            ...
            className={`rounded-full px-5 py-2 text-sm font-semibold capitalize transition-colors ${
              view === v ? "bg-brick text-cream" : "text-espresso/70 hover:bg-cream-deep"
            }`}
          >
            {v}
          </button>
      ...
      <div className="mt-6 space-y-3">
        ...
        {rows?.length === 0 && (
          <div className="rounded-2xl border-2 border-dashed border-sand p-6">
            <p className="text-espresso/60">
              No reviews yet. You can migrate the original built-in sample reviews to get started.
            </p>
            <button
              type="button"
        ...
        {visible.map((r) => (
          <div key={r._id} className="rounded-2xl border-2 border-sand bg-cream p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-espresso/55">
                  ...
                  {r.featured && (
                    <span className="ml-1 font-semibold text-brick">· ★ Featured</span>
                  )}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                {r.status === "pending" && (
                  <button
                    type="button"
                    className={btn.small}
                    onClick={() => void setStatus({ id: r._id, status: "approved" })}
                  >
                    Approve
                  </button>
                )}
                {r.status === "approved" && (
                  <>
                    ...
                    <button
                      type="button"
                      className={btn.small}
                      onClick={() => void setStatus({ id: r._id, status: "pending" })}
                    >
                      Unapprove
                    </button>
                  </>
                ...
                {r.status === "archived" ? (
                  ...
                  <button
                    type="button"
                    className={btn.small}
                    onClick={() => void setStatus({ id: r._id, status: "archived" })}
                  >
                    Archive
                  </button>
                )}
                <button
                  ...
                  onClick={() =>
                    confirmThen("Delete this review for good?", () => void remove({ id: r._id }))
                  }
                >
                  Delete
                </button>
