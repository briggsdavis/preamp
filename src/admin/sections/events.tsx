...
export function Events() {
  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-3">
        ...
        {rows?.length === 0 && (
          <div className="rounded-2xl border-2 border-dashed border-sand p-6">
            <p className="text-espresso/60">No events yet. Add one, or seed the original three.</p>
            <button
              type="button"
        ...
        {rows?.map((ev) => (
          ...
          >
            <div className="aspect-[4/3] w-20 shrink-0 overflow-hidden rounded-lg bg-cream-deep">
              {ev.images[0]?.url && (
                <img src={ev.images[0].url} alt="" className="h-full w-full object-cover" />
              )}
            </div>
            ...
            <button type="button" className={btn.small} onClick={() => setEditing(ev)}>
              Edit
            </button>
            <button
              type="button"
              className={btn.small}
              onClick={() => void setArchived({ id: ev._id, archived: !ev.archived })}
            >
            ...
            <button
              ...
              onClick={() =>
                confirmThen(`Delete "${ev.title}"?`, () => void remove({ id: ev._id }))
              }
            >
              Delete
            </button>
      ...
      {editing && (
        <EventForm existing={editing === "new" ? null : editing} onClose={() => setEditing(null)} />
      )}
    </div>
  )
}

function EventForm({ existing, onClose }: { existing: EventData | null; onClose: () => void }) {
  ...
  const [when, setWhen] = useState(existing ? toLocalInput(existing.startsAt) : "")
