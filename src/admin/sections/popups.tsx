...
import { PUBLIC_PAGES, POPUP_POSITIONS, DISPLAY_FREQUENCIES, ACTION_TRIGGERS } from "@/lib/cms"
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-espresso/60">
        One pop-up shows per on-screen position. Give a pop-up a schedule to run it for a set
        window; a later one automatically replaces it in the same spot.
      </p>
      {activateError && (
      ...
      <div className="mt-6 space-y-3">
        ...
        {rows?.length === 0 && <p className="text-espresso/55">No pop-ups yet.</p>}
        {rows?.map((p) => (
          <div key={p._id} className="rounded-2xl border-2 border-sand bg-cream p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="min-w-[12rem] flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <StatusBadge status={scheduleStatus(p.active, p.startsAt, p.endsAt)} />
                </div>
              ...
              <button
                type="button"
                className={btn.small}
                onClick={() => setExpanded((cur) => (cur === p._id ? null : p._id))}
              >
                {expanded === p._id ? "Hide stats" : "Stats"}
              </button>
              <button type="button" className={btn.small} onClick={() => setEditing(p as Popup)}>
                Edit
              </button>
              <button
                type="button"
                ...
                onClick={() =>
                  confirmThen(`Delete "${p.internalTitle}"?`, () => void remove({ id: p._id }))
                }
              >
                Delete
              </button>
      ...
      {editing && (
        <PopupForm existing={editing === "new" ? null : editing} onClose={() => setEditing(null)} />
      )}
    </div>
  )
}
...
function PopupForm({ existing, onClose }: { existing: Popup | null; onClose: () => void }) {
  ...
  const [position, setPosition] = useState<PopupPosition>(existing?.position ?? "center")
  const [frequency, setFrequency] = useState(existing?.frequency ?? "session")
  ...
  const [action, setAction] = useState(existing?.trigger.action ?? ACTION_TRIGGERS[0].key)
  const [emailCapture, setEmailCapture] = useState(existing?.emailCapture ?? false)
  const [backdropBlur, setBackdropBlur] = useState(existing?.backdropBlur ?? true)
  ...
  async function addMedia(e: React.ChangeEvent<HTMLInputElement>, type: "image" | "video") {
  ...
  return (
    <Modal title={existing ? "Edit pop-up" : "New pop-up"} onClose={onClose} wide>
      <div className="space-y-4">
        {/* Media */}
        <div>
          <label className={label}>Media - up to {MAX_IMAGES} images and one video</label>
          ...
          <div className="mt-2 flex gap-2">
            ...
            <label className={`${btn.small} cursor-pointer ${hasVideo ? "opacity-60" : ""}`}>
            ...
            {uploading && <span className="self-center text-sm text-espresso/55">Uploading…</span>}
          </div>
        </div>
