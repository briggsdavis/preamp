    ...
    <div>
      ...
      {data === undefined ? (
        ...
        <div className="space-y-6">
          ...
          {/* Today's snapshot */}
          <section>
            <h2 className="mb-3 font-groovy text-sm tracking-[0.2em] text-terracotta uppercase">
              Today at a glance
            </h2>
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              <SnapStat
                ...
                prev={data.today.prev.visitors}
              />
              <SnapStat
                label="Page Views"
                value={data.today.pageViews}
                prev={data.today.prev.pageViews}
              />
              <SnapStat
                label="Order Clicks"
                value={data.today.orderClicks}
                prev={data.today.prev.orderClicks}
              />
              <SnapStat
                label="Menu Clicks"
                value={data.today.menuClicks}
                prev={data.today.prev.menuClicks}
              />
            </div>
          </section>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            >
              ...
              <p className="mt-3 text-sm text-espresso/70">{nf.format(data.reviews.total)} total</p>
            ...
            {/* Campaigns */}
            <CommandCard title="Active Campaigns" onClick={() => onNavigate("announcements")}>
            ...
            <CommandCard
              ...
              onClick={() => onNavigate(data.topItem ? `menu-${data.topItem.menu}` : "menu-coffee")}
            >
              {data.topItem ? (
                <>
                ...
                <p className="text-sm text-espresso/50">No item views yet this week.</p>
              )}
            </CommandCard>
          </div>
...
function SnapStat({ label, value, prev }: { label: string; value: number; prev: number }) {
  ...
  return (
    <div className="rounded-2xl border-2 border-sand bg-cream p-4">
      ...
      <p className="mt-1 font-display text-3xl leading-none text-espresso">{nf.format(value)}</p>
      {showDelta ? (
...
function CommandCard({
  ...
  return (
    ...
    >
      <p className="mb-2 font-groovy text-xs tracking-[0.2em] text-terracotta uppercase">{title}</p>
