  ...
  return (
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <span className="mb-1 block text-xs font-semibold text-espresso/60">Starts</span>
          <input
            type="datetime-local"
            className={field}
            value={toLocalInput(startsAt)}
            onChange={(e) => onChange({ startsAt: fromLocalInput(e.target.value), endsAt })}
          />
        </div>
        <div>
          <span className="mb-1 block text-xs font-semibold text-espresso/60">Ends</span>
          <input
            type="datetime-local"
            className={field}
            value={toLocalInput(endsAt)}
            onChange={(e) => onChange({ startsAt, endsAt: fromLocalInput(e.target.value) })}
          />
        </div>
      </div>
      {invalid && (
