...
import { EditableText, useInlineEditingMode } from "@/components/cms/inline-editing"
        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {t.rating !== undefined && (
                <p className="mt-3 text-gold" aria-label={`${t.rating} out of 5 stars`}>
                  {"★".repeat(t.rating)}
                  <span className="text-espresso/20">{"★".repeat(5 - t.rating)}</span>
                </p>
              )}
              <footer className="mt-5 text-sm tracking-[0.2em] text-terracotta uppercase">
                {t.name} · {t.source}
              </footer>
            </motion.blockquote>
          </AnimatePresence>
