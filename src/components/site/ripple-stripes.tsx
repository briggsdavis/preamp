...
function waveHeight(i: number, count: number) {
  ...
  const detail = Math.abs(
    0.6 * Math.sin(i * 1.3 + 0.4) + 0.3 * Math.sin(i * 0.7 + 1.1) + 0.25 * Math.sin(i * 2.1 + 2),
  )
  ...
  const stripe = (i: number) => {
    ...
    const style: CSSProperties = {
      ...
      animation: `stripe-ripple ${4 + (k % 5) * 0.6}s ease-in-out ${
        k * 0.18
      }s infinite, stripe-shimmer ${5 + (k % 4)}s ease-in-out ${k * 0.12}s infinite`,
    }
    if (variant === "wave") {
    ...
    return (
      <span
        key={i}
        className={cn("block flex-1 rounded-full", variant === "wave" ? "self-center" : "h-full")}
        style={style}
      />
    )
  }
