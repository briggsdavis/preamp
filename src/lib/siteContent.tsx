...
export const DEFAULT_GLOBAL_CONTENT: GlobalContent = {
  ...
  socials: [
    { platform: "instagram", label: "@preamp.coffeestudio", url: "https://www.instagram.com/preamp.coffeestudio/" },
...
export function mergeContent<K extends CmsKey>(key: K, saved: unknown): ContentFor<K> {
  ...
  for (const [section, value] of Object.entries(saved)) {
    ...
    output[section] =
      base && value && typeof base === "object" && typeof value === "object" && !Array.isArray(base) && !Array.isArray(value)
        ? { ...base, ...value }
        : value;
  }
...
export function GlobalContentProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <GlobalContentContext.Provider value={content}>
      {children}
