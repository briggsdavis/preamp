...
function Carousel({
  ...
  const go = (dir: number) => setI((cur) => (cur + dir + images.length) % images.length)
      ...
      {images.length > 1 && (
        <>
          <button
            ...
            className="absolute top-1/2 left-2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-espresso/60 text-cream transition-colors hover:bg-espresso"
          >
            ‹
          </button>
          <button
            ...
            className="absolute top-1/2 right-2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-espresso/60 text-cream transition-colors hover:bg-espresso"
          >
            ›
          </button>
...
function EventModal({ event, onClose }: { event: EventDoc; onClose: () => void }) {
  useEffect(() => {
    ...
    >
      ...
      >
        {event.images.length > 0 && (
          <Carousel images={event.images} alt={event.title} className="aspect-[4/3]" />
        )}
        <div className="p-6">
          <p className="font-groovy text-sm tracking-[0.2em] text-terracotta uppercase">
      ...
      <div className="overflow-hidden rounded-lg border border-espresso/15">
        <div className="grid grid-cols-7 bg-brick">
          {WEEKDAYS.map((d) => (
            <div
              key={d}
              className="py-2.5 text-center font-semibold tracking-wide text-cream uppercase"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7">
          {cells.map((d, i) => {
            ...
            const dayEvents = events.filter((ev) => sameDay(new Date(ev.startsAt), d))
            return (
              <div
                key={i}
                className={`min-h-[96px] border-r border-b border-espresso/10 p-2 md:min-h-[120px] ${
                  i % 7 === 0 ? "border-l" : ""
...
function UpcomingEvents({ events }: { events: EventDoc[] }) {
  ...
  if (sorted.length === 0) {
    return (
      <p className="text-center text-espresso/60">No upcoming events right now. Check back soon!</p>
    )
  }
    ...
    <div className="space-y-8">
      {sorted.map((ev, i) => {
        ...
        return (
          ...
          >
            <Carousel images={ev.images} alt={ev.title} className="aspect-[4/3] self-start" />
            <div className="p-6 md:p-8">
              <p className="font-groovy text-sm tracking-[0.2em] text-terracotta uppercase">
                {formatEventDate(date)} · {formatEventTime(date)}
              </p>
              <h3 className="mt-2 font-display text-3xl text-espresso">{ev.title}</h3>
