 ...
 *
 * `bun run build` runs `vite build` then this script. It pulls the live menu
 * from Convex and writes a static
...
const MENU_KINDS = ["coffee", "food"] as const

type MenuKind = (typeof MENU_KINDS)[number]
type Menu = FunctionReturnType<typeof api.menu.getMenu>
type MenuItem = Menu["sections"][number]["items"][number]
...
export function renderItem(template: string, { kind, item, tagLabels }: RenderItemOptions): string {
  const path = `/menu/${kind}/${item.slug}`
  const canonical = absolute(path)
  const image = absolute(item.images?.find((i) => i.url)?.url || "")
  const title = `${item.name} · ${kind === "coffee" ? "Coffee" : "Food"} · Pre Amp Coffee Studio`
...
async function main(): Promise<void> {
  ...
  let tagLabelByKey: Record<string, string> = {}
  try {
    const tags = await client.query(api.dietaryTags.list, {})
  ...
  for (const kind of MENU_KINDS) {
    let data
    try {
      data = await client.query(api.menu.getMenu, { menu: kind })
    } catch (e) {
      console.warn(`[prerender] Failed to load ${kind} menu:`, errorMessage(e))
      continue
    }
  ...
  if (SITE_URL) {
    ...
    console.warn("[prerender] No VITE_SITE_URL set - skipping sitemap.xml (needs absolute URLs).")
  }
}

// Only run as a CLI (`bun scripts/prerender.ts`), not when imported for tests.
if (process.argv[1]?.endsWith("prerender.ts")) {
  main().catch((e) => {
    // Never fail the build over prerendering - the SPA still serves everything.
    console.warn("[prerender] Skipped due to error:", errorMessage(e))
  })
}
