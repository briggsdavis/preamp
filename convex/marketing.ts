...
function pickWinner<T extends Schedulable & { _creationTime: number }>(candidates: T[]): T | null {
...
export const updateAnnouncement = mutation({
  ...
  handler: async (ctx, { id, ...fields }) => {
    ...
    if (bar?.active) {
      ...
      const all = await ctx.db.query("announcements").collect()
      const clash = all.find((a) => a._id !== id && a.active && windowsOverlap(a, fields))
      if (clash) {
...
export const setAnnouncementActive = mutation({
  ...
  handler: async (ctx, { id, active }) => {
    ...
    if (active) {
      const all = await ctx.db.query("announcements").collect()
      const clash = all.find((a) => a._id !== id && a.active && windowsOverlap(a, bar))
      if (clash) {
        throw new ConvexError(
...
export const listActivePopups = query({
  ...
  handler: async (ctx) => {
    ...
    return await Promise.all([...byPosition.values()].map((p) => withMediaUrls(ctx, p)))
  },
})
  ...
  handler: async (ctx, { id, ...fields }) => {
    ...
    if (popup?.active) {
      ...
      const clash = all.find(
        (p) =>
          p._id !== id && p.active && p.position === fields.position && windowsOverlap(p, fields),
      )
      if (clash) {
        throw new ConvexError(
...
export const setPopupActive = mutation({
  ...
  handler: async (ctx, { id, active }) => {
    ...
    if (active) {
      ...
      const clash = all.find(
        (p) =>
          p.active && p._id !== id && p.position === popup.position && windowsOverlap(p, popup),
      )
      if (clash) {
        throw new ConvexError(
